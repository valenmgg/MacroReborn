...
//     el argumento.
//   - Se puede cachear un año, porque la URL es el contenido.
...
test("el Content-Length coincide con los bytes enviados", async () => {
  const r = await llamarComoServidor({ v: huellaDe(PNG_B) });
...
test("se puede cachear un año, porque la URL es el contenido", async () => {
  const r = await llamarComoServidor({ v: huellaDe(PNG_A) });
...
test("dos prendas con el mismo dibujo comparten una sola URL", async () => {
  ...
  const r = await llamarComoServidor({ v: filas.rows[0].sha256 });
  assert.equal(r.codigo, 200);
...
test("una huella que no existe da 404", async () => {
  ...
  const r = await llamarComoServidor({ v: inventada });
